...
public final class GuildMessageFilterPatterns {
    ...
    public static final Pattern OTHER_GUILD_VICTORIOUS = Pattern.compile("(?<guild>.+?) was victorious!");
    ...
    public static final Pattern ACTIVE_ATTACK_CANCELED = Pattern.compile("Your active attack was canceled and refunded to your headquarter.");

    // Economy
    public static final Pattern BONUS_SET = Pattern.compile("(?<player>.+?) set (?<bonus>.+?) bonus to level (?<level>\\d+) on (?<territory>.+?)");
    public static final Pattern BONUS_REMOVED = Pattern.compile("(?<player>.+?) removed (?<bonus>.+?) bonus from (?<territory>.+?)");
    public static final Pattern BONUSES_CHANGED =
            Pattern.compile("(?<player>.+?) changed (?<amount>\\d+) bonuses on (?<territory>.+?)");
    public static final Pattern UPGRADE_SET = Pattern.compile("(?<player>.+?) set (?<upgrade>.+?) upgrade to level (?<level>\\d+) on (?<territory>.+?)");
    public static final Pattern UPGRADE_REMOVED = Pattern.compile("(?<player>.+?) removed (?<upgrade>.+?) upgrade from (?<territory>.+?)");
    ...
    public static final Pattern TERRITORY_TAX_CHANGED = Pattern.compile(
            "(?<player>.+?) changed the tax of (?<territory>.+?) to (?<tax>.+?)%");
    ...
    public static final Pattern LOADOUT_APPLIED = Pattern.compile("(?<player>.+?) applied the loadout (?<loadout>.+?) on (?<territory>.+?)");
    public static final Pattern TRIBES_SCHEDULE_STOPPED = Pattern.compile("(?<guild1>.+?) stopped scheduling (?<resource>.+?) to (?<guild2>.+?)");
    public static final Pattern TRIBES_SCHEDULED = Pattern.compile("(?<guild1>.+?) scheduled (?<resourceIcon>.+?) (?<amount>.+?) (?<resource>.+?) per hour to (?<guild2>.+?)");
    public static final Pattern GLOBAL_BORDER_CHANGED = Pattern.compile("(?<player>.+?) changed the global borders to (?<style>.+?)");
    public static final Pattern BORDERS_CHANGED = Pattern.compile("(?<player>.+?) changed the borders of (?<territory>.+?) to (?<style>.+?)");

    // Reward
    public static final Pattern REWARD = Pattern.compile("(?<player>.+?) rewarded (?<reward>.+?) to (?<recipient>.+?)");

    // Bank
    public static final Pattern EVERYONE_BANK_DEPOSIT = Pattern.compile("(?<player>.+?) deposited (?<item>.+?) to the Guild Bank \\(Everyone\\)");
    public static final Pattern EVERYONE_BANK_WITHDRAWAL = Pattern.compile("(?<player>.+?) withdrew (?<item>.+?) from the Guild Bank \\(Everyone\\)");
    public static final Pattern HIGH_RANKED_BANK_DEPOSIT = Pattern.compile("(?<player>.+?) deposited (?<item>.+?) to the Guild Bank \\(High Ranked\\)");
    public static final Pattern HIGH_RANKED_BANK_WITHDRAWAL = Pattern.compile("(?<player>.+?) withdrew (?<item>.+?) from the Guild Bank \\(High Ranked\\)");
